	...
	while(i<gheight)
	{
		...
		while(j<gwidth)
		{
			if(tiles[i][j].valid==1||tiles[i][j].valid==4)
			{
				...
				DDraw_Draw_Surface(gridTiles[tiles[i][j].ur],x+10,y,10,10,screen,1);
				DDraw_Draw_Surface(gridTiles[tiles[i][j].bl],x,y+10,10,10,screen,1);
				DDraw_Draw_Surface(gridTiles[tiles[i][j].br],x+10,y+10,10,10,screen,1);
			}
			else if(tiles[i][j].valid==2||tiles[i][j].valid==3)
				DDraw_Draw_Surface(gridTiles[tiles[i][j].ul],X_OFFSET+j*20,Y_OFFSET+i*20,20,20,screen,1);
			j++;
			x+=20;
		}
		y+=20;
...
int Level::checkGrid(int i,int j)
{
	...
	if(i>=G_HEIGHT || j>=G_WIDTH)
		return 0;
	if(tiles[i][j].valid==1)return 1;
	if(tiles[i][j].valid==4)return 1;
	else return 0;
}
		...
		while(1)
		{
			...
			if(fi==4)
			{
				...
				*/
				...
				fprintf(dbg,"foodLoc[%d]=[%d,%d]\n",foodLocCount,foodLocIs[foodLocCount],foodLocJs[foodLocCount]);
				this->foodLocCount++;
...
int Level::BFS(loc * target, nqueue * q,int visited[G_HEIGHT][G_WIDTH], int mark)
{
	loc current;
	int pathNum=-1;
...
int Level::limitedDFS(loc * target,loc * stack,int * instack,int * path, int *inpath,int visited[G_HEIGHT][G_WIDTH],int cutoff)
{
	int pathback =*inpath;
	loc current;
	(*instack)--;
...
int Level::canGo(int visited[G_HEIGHT][G_WIDTH],int cutoff,int i, int j, int dir)
{
	switch(dir)
	{
		/*
...
inline int Level::canGo2(int visited[G_HEIGHT][G_WIDTH],int cutoff,int i, int j, int dir)
{
	switch(dir)
	{
	case D_UP:
