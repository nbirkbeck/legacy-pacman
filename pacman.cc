	...
	*/	
	...
	if(fruit.valid)
	{
		...
		if(!fruit.hasLTGoal)
		{
			...
			if(-1!=level->findPath(fruit.y/20,fruit.x/20,
				level->fruitEnd.i,level->fruitEnd.j))
			{
				fruit.hasLTGoal=1;
		...
		if(!enemies[ei].hasLTGoal)
		{
			...
			*/
			...
			while(!done)
			{
				...
				{
					if(-1!=level->findPath2(enemies[ei].y/20,enemies[ei].x/20,ri,rj))
						break;
				}
				rj=rand()%(G_WIDTH);
				ri=rand()%(G_HEIGHT);
...
inline void drawEnemy(int ei)
{
	...
	DDraw_Draw_Surface(enemies[ei].textures[enemies[ei].state-offset][pacman.frameOrder[pacman.frame]],
                            X_OFFSET+enemies[ei].x-2,Y_OFFSET+enemies[ei].y-2,24,24,surface,1);
}
...
inline void drawFruit()
{
	if(fruit.valid)
          DDraw_Draw_Surface(fruit.fruitTextures[fruit.fruiti],
                              X_OFFSET+fruit.x-2,Y_OFFSET+fruit.y-2,24,24,surface,1);
}
	...
	if(pacman.state==P_NORMAL)
	{
		...
		*/
		...
		if(fruit.valid)
		{
			...
			//
			if(abs(pacman.x-fruit.x)<20
				&&
				abs(pacman.y-fruit.y)<20)
			{
		...
		if(pacman.x>=540)pacman.x=0;
		if(pacman.y>=440)pacman.y=0;
		if(pacman.x<0)pacman.x=540;
		if(pacman.y<0)pacman.y=440;
	}
	else
	{
		weak--;
...
{
  DDraw_Draw_Surface(pacman.textures[pacman.direction][pacman.frameOrder[pacman.frame]],
...
inline void drawFloatingScore()
{
	if(score.framesLeft>0)
	{
		...
		while(i>=0)
		{
			DDraw_Draw_Surface(digits[score.digits[i]],
				score.x + offset, score.y + Y_OFFSET,16,14,surface,1);
			score.y-=dec;
			offset+=12;
			i--;
		}
...
inline int drawScoreBoard()
{
	...
	while(i>=0)
	{
		DDraw_Draw_Surface(bigDigits[scoreDigits[i]],
		x,0,18,20,surface,1);
		i--;
		x+=16;
	}
	...
	while(i<lives[0])
	{
		DDraw_Draw_Surface(pacman.textures[D_LEFT][3],
			x,456,24,24,surface,0);
		i++;
		x+=30;
	}
	DrawFPS();
...
int Game_Intro()
{
	...
	DDraw_Draw_Surface(pacmanTitle,
		135,y,400,200,surface,1);
	
	DDraw_Draw_Surface(startItem,
		255,menuStartY-y+100,120,yspace,surface,1);
	DDraw_Draw_Surface(hiScoreItem,
		255,menuStartY+yspace-y+100,120,yspace,surface,1);
	DDraw_Draw_Surface(exitItem,
		255,menuStartY+2*yspace-y+100,120,yspace,surface,1);
	...
	DDraw_Draw_Surface(enemies[0].textures[estate][aframe],
		ex,ey,24,24,surface,1);
	DDraw_Draw_Surface(enemies[1].textures[E_NORMAL][aframe],
		ex+30,ey+10,24,24,surface,1);

	DDraw_Draw_Surface(enemies[2].textures[estate][aframe],
		640-ex-24,ey,24,24,surface,1);
	DDraw_Draw_Surface(enemies[3].textures[E_NORMAL][aframe],
		640-ex-54,ey+10,24,24,surface,1);


	DDraw_Draw_Surface(fruit.fruitTextures[3],
		ex,y+150,24,24,surface,1);
	DDraw_Draw_Surface(fruit.fruitTextures[2],
		ex+30,y+160,24,24,surface,1);

	DDraw_Draw_Surface(fruit.fruitTextures[0],
		640-ex-24,y+150,24,24,surface,1);
	DDraw_Draw_Surface(fruit.fruitTextures[1],
		640-ex-54,y+160,24,24,surface,1);
	...
	else
	{
		...
		DDraw_Draw_Surface(pacman.textures[D_RIGHT][3],
		255,menuStartY+selected*yspace+8,24,24,surface,1);
		if(keys[SDLK_DOWN])
		{
			keys[SDLK_DOWN]=0;
			selected++;
...
int Game_Paused()
{
	...
	DDraw_Draw_Surface(pacman.textures[D_RIGHT][3],
	260,menuStartY+selected*yspace+8,24,24,surface,1);
	DDraw_Draw_Surface(returnItem,
	260,menuStartY,120,yspace,surface,1);
	DDraw_Draw_Surface(exitItem,
	260,menuStartY+yspace,120,yspace,surface,1);
...
{	
	...
	while(i<strlen(levelNames[levelNumber]))
	{
		if(levelNames[levelNumber][i]!=' ')
			DDraw_Draw_Surface(font[levelNames[levelNumber][i]-65],count+28*i,210,32,32,surface,1);
		i++;
	}
	if(halflen == -1)
	{
...
int Game_Init()
{
   ...
   */
	...
	*/
	enemies[0].textures[E_NORMAL][0]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_NORMAL][1]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_NORMAL][2]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_NORMAL][3]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	...
	/*
		Enemy 0's weak texture is loaded, which is the same as the others weak texture
	*/
	enemies[0].textures[E_WEAK][0]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_WEAK][1]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_WEAK][2]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_WEAK][3]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	...
	/*
		Enemy 0's dead texture is loaded, which is the same as the others weak texture
	*/
	enemies[0].textures[E_DEAD][0]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_DEAD][1]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_DEAD][2]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[0].textures[E_DEAD][3]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	...
	enemies[1].textures[E_NORMAL][0]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[1].textures[E_NORMAL][1]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[1].textures[E_NORMAL][2]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[1].textures[E_NORMAL][3]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);

	NDDraw_Draw_Surface(test,0,24,24,24,enemies[1].textures[E_NORMAL][0],0);
	NDDraw_Draw_Surface(test,24,24,24,24,enemies[1].textures[E_NORMAL][1],0);
	NDDraw_Draw_Surface(test,48,24,24,24,enemies[1].textures[E_NORMAL][2],0);
	NDDraw_Draw_Surface(test,72,24,24,24,enemies[1].textures[E_NORMAL][3],0);

	enemies[2].textures[E_NORMAL][0]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[2].textures[E_NORMAL][1]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[2].textures[E_NORMAL][2]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[2].textures[E_NORMAL][3]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);

	NDDraw_Draw_Surface(test,0,48,24,24,enemies[2].textures[E_NORMAL][0],0);
	NDDraw_Draw_Surface(test,24,48,24,24,enemies[2].textures[E_NORMAL][1],0);
	NDDraw_Draw_Surface(test,48,48,24,24,enemies[2].textures[E_NORMAL][2],0);
	NDDraw_Draw_Surface(test,72,48,24,24,enemies[2].textures[E_NORMAL][3],0);

	enemies[3].textures[E_NORMAL][0]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[3].textures[E_NORMAL][1]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[3].textures[E_NORMAL][2]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);
	enemies[3].textures[E_NORMAL][3]=SDL_CreateRGBSurface(0, 24,24,32, 0, 0, 0, 0);

	NDDraw_Draw_Surface(test,0,72,24,24,enemies[3].textures[E_NORMAL][0],0);
	NDDraw_Draw_Surface(test,24,72,24,24,enemies[3].textures[E_NORMAL][1],0);
	NDDraw_Draw_Surface(test,48,72,24,24,enemies[3].textures[E_NORMAL][2],0);
	NDDraw_Draw_Surface(test,72,72,24,24,enemies[3].textures[E_NORMAL][3],0);
